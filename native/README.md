...
| 新增 | 說明 |
|------|------|
| **Pivot 標題條** | 當前頁大字標題（46sp Light）+ **下一頁標題從右側探出且可點跳頁**；滑動時標題隨頁面連續位移與縮放，探出部分被邊界自然裁切（不是「favo…」省略號） |
| **磁貼 3D 傾斜** | 按下時磁貼朝**觸控點**做 ±10° rotateX/rotateY 傾斜（不只是縮放），並觸發觸感回饋——WP 招牌按壓手感 |
| **磁貼角標** | 右上角圓形數字角標（WP Live Tile badge），收藏數 / 實時班次數 |
| **App Bar 自動隱藏** | 向下捲動累計 48dp 收起、向上捲動回來（
estedScroll 統一接管，對應概念圖 `.appbar.hidden`） |
| **搜尋／重新整理移入 App Bar** | 真 WP8 把這類動作放 App Bar，頂欄留給 Pivot 標題（也解決標題被按鈕擠壓的問題） |
| **WP 進度條接上真實狀態** | 頂欄下方 3px 不確定進度條，搜尋中／路線圖載入／壽司郎載入時才會出現 |
| **WP 載入點** | `Wp8LoadingDots`（五顆方點依序亮起，WP 沒有轉圈圈）取代「載入中…」 |
