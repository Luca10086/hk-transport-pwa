...
### MiMo 分工原则（用户指令 · 2026-03）

**简单的、输入明确的产出型任务优先派给 MiMo（原生模型源 `provider: 'xiaomi'`）**：图标设计（单色线性 SVG 草稿与 2–3 变体）、UI/UX 排版取值建议（间距/字级/配色/token 对照）、短片段样式对照设计文档、文案/磁贴标语、方案评审与找漏。
**主代理自己做**：跨文件落地（html+css+js 联动）、交互逻辑与数据模型、多皮肤并存、git/测试/真实数据验证、以及 **MiMo 产出的审查修正（double-check 是主代理责任）**，MiMo 产出必须经主代理核实后才落地。
判断主动权在主代理：看任务是否「一屏讲得清、是否需跨文件读码、风险高低」，是则派 MiMo，否则自己做。
